turtles-own
  [ sick?                ;; if true, the turtle is infectious
    ...
    vaccinated? ]         ;; vaccinated or not

globals
  [ %infected            ;; what % of the population is infectious
    %immune              ;; what % of the population is immune
    %vac                 ;; what % of the population is vaccinated
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ sick? ] / count turtles) * 100
      ...
      set %vac (count turtles with [ vaccinated? ] / count turtles) * 100]
end

to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value immune? [ grey ] [ ifelse-value vaccinated? [ blue ] [ green ] ] ] ]
end
  ...
  if count turtles < carrying-capacity and random-float 100 < chance-reproduce
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        set vaccinated? false
        if random-float 100 < pop-vaccinated
          [ set vaccinated? true ]
        get-healthy ] ]
end
...
"vac'd" 1.0 0 -13345367 true "" "plot count turtles with [ vaccinated? ]"
